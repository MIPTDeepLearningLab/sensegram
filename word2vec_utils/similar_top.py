...
def dists2neighbours(vec, dists, indices, topn):
    ...
    res = OrderedDict()
    for i in range(len(indices)):
        sims = best[i, np.argsort(
            -dists[i, best[i]])]  # sims is a list of indices (in relation to syn0norm) of nearest neighbours
...
def similar_top_opt3(vec, words, topn=200, nthreads=4, freq=None):
    ...
    if nthreads == 1:
        ...
        print(
        "dists2neighbours for %d words in %d threads, batchsize=%d" % (len(indices), nthreads, batchsize), file=stderr)
        def ppp(i):
...
def print_similar(out, vectors, batch, mindist=None, only_letters=False, pairs=False, freq=None):
    try:
        for word, ns in similar_top_opt3(vectors.wv, batch, freq=freq).items():
            sims = []
            for w, d in ns:
                if (mindist is None or d >= mindist) and (not only_letters or re_only_letters.match(w) is not None):
                    ...
                    print("%s: SKIPPED\t%s\t%r" % (word, w, d), file=stderr)

                if pairs:
                    print('\n'.join(("%s\t%s\t%f" % (word, w, d) for w, d in sims)), file=out)
                else:
                    print("%s\t%s" % (word, ','.join(("%s:%f" % (w, d) for w, d in sims))), file=out)
...
def init(fvec, output="", only_letters=False, vocab_limit=None, pairs=False, batch_size=1000, word_freqs=None):
    ...
    print("Vectors shape is: ", vectors.wv.syn0norm.shape, file=stderr)

    vocab_size = len(vectors.wv.vocab)
    ...
    words = vectors.wv.index2word[:vocab_size]
    ...
    with codecs.open(output, 'w') if output else stdout as out:
...
def main():
    ...
    parser.add_argument('-only_letters',
                        help='Skip words containing non-letter symbols from stding / similar words.',
                        action="store_true")
    parser.add_argument("-vocab_limit",
                        help="Collect neighbours only for specified number of most frequent words. By default use all words.",
                        default=None, type=int)
    parser.add_argument('-pairs',
                        help="Use pairs format: 2 words and distance in each line. Otherwise echo line is a word and all it's neighbours with distances.",
                        action="store_true")
    parser.add_argument('-batch-size', help='Batch size for finding neighbours.', default="1000")
    parser.add_argument('-word_freqs', help="Weight similar words by frequency. Pass frequency file as parameter",
                        default=None)

    args = parser.parse_args()

    init(args.vectors, output=args.output, only_letters=args.only_letters, vocab_limit=args.vocab_limit,
         pairs=args.pairs, batch_size=int(args.batch_size), word_freqs=args.word_freqs)
